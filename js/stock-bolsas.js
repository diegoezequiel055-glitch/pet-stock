...
async function cargarProductos() {
  try {
    const snap = await db.collection('productos').orderBy('marca').get();
    ...
    return '<div style="margin-bottom:8px;border-radius:10px;overflow:hidden">'
      ...
      + '<div style="display:none;background:#131c2e;padding:12px 14px;border:1px solid #1e3a5f;border-top:none;border-radius:0 0 10px 10px">'
        + '<div style="display:flex;gap:8px;align-items:center">'
          ...
          + '<button class="btn-basura btn-basura-sm" onclick="eliminarProducto(\'' + p.id + '\',\'' + (p.marca + ' ' + p.nombre).replace(/'/g,'') + '\')" title="Eliminar producto">&#128465;&#65039;</button>'
        + '</div>'
      + '</div>'
    + '</div>';
  }).join('');
...
async function agregarLote(e) {
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      t.update(productoRef, { stockTotal: stockActual + cantidad, ultimoCosto: costo });
    });
...
async function registrarVenta(e) {
  ...
  try {
    ...
    const lotes       = lotesSnap.docs.map(d => ({ id: d.id, ref: d.ref, ...d.data() })).filter(l => l.cantidadRestante > 0);
    ...
    await db.runTransaction(async (t) => {
      for (const lu of loteUpdates) { t.update(lu.ref, { cantidadRestante: lu.nuevaCantidad }); }
      t.update(productoRef, { stockTotal: firebase.firestore.FieldValue.increment(-cantidadVendida) });
      const ventaRef = db.collection('ventas').doc();
      t.set(ventaRef, {
        productoId: productoSeleccionadoId, nombreProducto: prod.marca + ' - ' + prod.nombre,
        cantidad: cantidadVendida, precioVenta, totalVenta: precioVenta * cantidadVendida,
        costoFIFO: costoTotalFIFO, ganancia: gananciaTotal, gananciaUnit: gananciaUnitaria,
        cliente: cliente || '-', tipo: 'bolsa', fecha: firebase.firestore.FieldValue.serverTimestamp()
      });
    });
    mostrarAlerta('Venta registrada. Ganancia: ' + formatPrecio(gananciaTotal), 'success');
...
async function verLotes(productoId) {
  ...
  try {
    const snap  = await db.collection('productos').doc(productoId).collection('lotes').orderBy('fechaCompra','asc').get();
    ...
    if (snap.empty) {
      ...
      tbody.innerHTML = snap.docs.map(doc => {
        ...
        return '<tr class="' + (agotado ? 'lote-agotado' : '') + '">'
          ...
          + '<td data-label=""><button class="btn-basura btn-basura-sm" onclick="eliminarLote(\'' + productoId + '\',\'' + doc.id + '\',' + l.cantidadRestante + ')" title="Eliminar lote">&#128465;&#65039;</button></td>'
          + '</tr>';
      }).join('');
    }
...
async function eliminarLote(productoId, loteId, cantidadRestante) {
  if (!confirm('Eliminar este lote?\nSe descontaran ' + cantidadRestante + ' unidades del stock.\nEsta accion no se puede deshacer.')) return;
  try {
    const productoRef = db.collection('productos').doc(productoId);
    await db.runTransaction(async (t) => {
      t.delete(productoRef.collection('lotes').doc(loteId));
      t.update(productoRef, { stockTotal: firebase.firestore.FieldValue.increment(-cantidadRestante) });
    });
...
async function eliminarProducto(productoId, nombreProducto) {
  if (!confirm('Eliminar "' + nombreProducto + '" y todos sus lotes?\nEsta accion no se puede deshacer.')) return;
  try {
    const productoRef = db.collection('productos').doc(productoId);
