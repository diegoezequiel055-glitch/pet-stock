// =============================================
// accesorios.js — Pet Stock
// =============================================
let accesoriosCache = [];
let accSeleccionadoId = '';

const CATEGORIAS = {
  accesorio:'🧸 Accesorio', farmacia:'💊 Farmacia', plastico:'🪣 Plastico',
  colchoneta:'🛏️ Colchoneta', higiene:'🧴 Higiene', otro:'📦 Otro'
};
...
async function cargarAccesorios() {
  ...
  try {
    ...
    for (const doc of snap.docs) {
      try {
        ...
        if (data.tieneVariantes) {
          items.push({ id: doc.id, _esPadre: true, _esVariante: false,
            nombre: data.nombre||'(sin nombre)', marca: data.marca||'',
            categoria: data.categoria||'otro', tieneVariantes: true });
          try {
            const varSnap = await db.collection('accesorios').doc(doc.id).collection('variantes').get();
            varSnap.docs.forEach(vDoc => {
              const v = vDoc.data();
              items.push({ id: vDoc.id, padreId: doc.id, _esPadre: false, _esVariante: true,
                nombrePadre: data.nombre||'', nombre: (data.nombre||'')+' — '+(v.nombre_variante||''),
                marca: data.marca||'', categoria: data.categoria||'otro',
                costo: v.costo||0, precioVenta: v.precioVenta||0, stock: v.stock||0,
                nombre_variante: v.nombre_variante||'' });
            });
          } catch(errVar) { console.warn('Error variantes doc '+doc.id, errVar); }
        } else {
          items.push({ id: doc.id, _esPadre: false, _esVariante: false,
            nombre: data.nombre||'(sin nombre)', marca: data.marca||'',
            categoria: data.categoria||'otro', costo: data.costo||0,
            precioVenta: data.precioVenta||0, stock: data.stock||0 });
        }
      } catch(errDoc) { console.warn('Error doc '+doc.id, errDoc); }
    }
    ...
    if (tbody) tbody.innerHTML = '<p style="text-align:center;color:var(--rojo);padding:24px">Error al cargar. Recarga la pagina.</p>';
...
function renderTablaAccesorios(lista) {
  ...
  var padres  = lista.filter(a => a._esPadre);
  var simples = lista.filter(a => !a._esPadre && !a._esVariante);
  var getV    = pid => lista.filter(a => a._esVariante && a.padreId === pid);
    ...
    html += '<div style="margin-bottom:8px;border-radius:10px;overflow:hidden">'
      ...
      +'<div style="'+PANEL+'">'
        ...
        +'<div style="display:flex;gap:8px;margin-top:4px;align-items:center">'
          ...
          +'<button class="btn-basura btn-basura-sm" onclick="eliminarPadreCompleto(\''+p.id+'\',\''+p.nombre.replace(/'/g,'')+'\''+')" title="Eliminar grupo">&#128465;&#65039;</button>'
        +'</div>'
      +'</div>'
    +'</div>';
  });
    ...
    html += '<div style="margin-bottom:8px;border-radius:10px;overflow:hidden">'
      ...
      +'<div style="'+PANEL+'">'
        ...
        +'<div style="display:flex;gap:8px;align-items:center">'
          ...
          +'<button class="btn-basura btn-basura-sm" onclick="eliminarAccesorio(\''+a.id+'\',\''+a.nombre.replace(/'/g,'')+'\''+')" title="Eliminar">&#128465;&#65039;</button>'
        +'</div>'
      +'</div>'
    +'</div>';
  });
...
function actualizarResumen(lista) {
  var items  = lista.filter(a => !a._esPadre);
  ...
  if (elSin)   elSin.textContent   = items.filter(a => (a.stock||0) <= 0).length;
  if (elValor) elValor.textContent  = formatPrecio(items.reduce((s,a) => s + (a.costo*(a.stock||0)), 0));
}
...
function toggleVariantes() {
  ...
  if (check.checked) { simples.style.display='none'; varDiv.style.display='block'; }
  else               { simples.style.display='block'; varDiv.style.display='none'; }
}
...
function agregarRenglonVariante() {
  ...
  div.innerHTML =
    '<button type="button" onclick="this.parentElement.remove()" style="position:absolute;top:6px;right:8px;background:none;border:none;font-size:18px;cursor:pointer;color:#c00">x</button>'
    +'<div class="form-fila">'
      +'<div class="form-grupo"><label>Variante *</label><input type="text" class="var-nombre" placeholder="Ej: 4-10kg" required></div>'
      +'<div class="form-grupo"><label>Costo ($)</label><input type="number" class="var-costo" min="0" step="0.01" placeholder="0"></div>'
      +'<div class="form-grupo"><label>Precio venta ($)</label><input type="number" class="var-precio" min="0" step="0.01" placeholder="0"></div>'
      +'<div class="form-grupo"><label>Stock</label><input type="number" class="var-stock" min="0" value="0"></div>'
    +'</div>';
...
async function agregarAccesorio(e) {
  ...
  if (!nombre || !categoria) { mostrarAlerta('Completa nombre y categoria', 'warning'); btn.disabled=false; return; }
  try {
    if (tieneVar) {
      var renglones = document.querySelectorAll('#variantes-lista .variante-renglon');
      if (renglones.length === 0) { mostrarAlerta('Agrega al menos una variante', 'warning'); btn.disabled=false; return; }
      for (var i=0; i<renglones.length; i++) {
        if (!renglones[i].querySelector('.var-nombre').value.trim()) { mostrarAlerta('Completa el nombre de todas las variantes', 'warning'); btn.disabled=false; return; }
      }
      var padreRef = await db.collection('accesorios').add({ nombre, marca, categoria, tieneVariantes: true, creadoEn: firebase.firestore.FieldValue.serverTimestamp() });
      var batch = db.batch();
      renglones.forEach(r => {
        var varRef = db.collection('accesorios').doc(padreRef.id).collection('variantes').doc();
        batch.set(varRef, { padreId: padreRef.id, nombre_variante: r.querySelector('.var-nombre').value.trim(),
          costo: parseFloat(r.querySelector('.var-costo').value)||0, precioVenta: parseFloat(r.querySelector('.var-precio').value)||0,
          stock: parseInt(r.querySelector('.var-stock').value)||0 });
      });
      await batch.commit();
      mostrarAlerta('"'+nombre+'" con variantes guardado', 'success');
    } else {
      await db.collection('accesorios').add({ nombre, marca, categoria,
    ...
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al guardar', 'error'); }
  finally { btn.disabled = false; }
}

function abrirModalSumarStock(id) {
  accSeleccionadoId = id;
  var acc = accesoriosCache.find(a => a.id === id);
...
function abrirModalSumarStockVariante(padreId, varId) {
  ...
  var acc = accesoriosCache.find(a => a.id === varId && a._esVariante);
  document.getElementById('sum-acc-nombre').textContent   = acc ? acc.nombre : '';
  document.getElementById('sum-stock-actual').textContent = acc ? acc.stock  : 0;
  document.getElementById('sum-cantidad').value = '';
...
async function sumarStock(e) {
  ...
  if (!cantidad || cantidad <= 0) { mostrarAlerta('Ingresa una cantidad valida', 'warning'); btn.disabled=false; return; }
  var padreId = document.getElementById('modal-sumar-stock').dataset.padreId;
  try {
    if (padreId) {
      await db.collection('accesorios').doc(padreId).collection('variantes').doc(accSeleccionadoId).update({ stock: firebase.firestore.FieldValue.increment(cantidad) });
    } else {
      await db.collection('accesorios').doc(accSeleccionadoId).update({ stock: firebase.firestore.FieldValue.increment(cantidad) });
    }
    mostrarAlerta('+'+cantidad+' unidades sumadas', 'success');
    cerrarModal('modal-sumar-stock');
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al actualizar el stock', 'error'); }
  finally { btn.disabled = false; }
}
...
function abrirModalEditarPadre(id) {
  accSeleccionadoId = id;
  var acc = accesoriosCache.find(a => a.id === id && a._esPadre);
  if (!acc) return;
  document.getElementById('edit-nombre').value    = acc.nombre;
  document.getElementById('edit-marca').value     = acc.marca || '';
  document.getElementById('edit-categoria').value = acc.categoria;
...
async function guardarEdicionAcc(e) {
  ...
  try {
    if (padreId === 'PADRE') {
      await db.collection('accesorios').doc(accSeleccionadoId).update({ nombre: document.getElementById('edit-nombre').value.trim(), marca: document.getElementById('edit-marca').value.trim(), categoria: document.getElementById('edit-categoria').value });
    } else if (padreId) {
      await db.collection('accesorios').doc(padreId).collection('variantes').doc(accSeleccionadoId).update({ nombre_variante: document.getElementById('edit-nombre').value.trim(), costo: parseFloat(document.getElementById('edit-costo').value)||0, precioVenta: parseFloat(document.getElementById('edit-precio').value)||0 });
    } else {
      await db.collection('accesorios').doc(accSeleccionadoId).update({ nombre: document.getElementById('edit-nombre').value.trim(), marca: document.getElementById('edit-marca').value.trim(), categoria: document.getElementById('edit-categoria').value, costo: parseFloat(document.getElementById('edit-costo').value)||0, precioVenta: parseFloat(document.getElementById('edit-precio').value)||0 });
    }
    mostrarAlerta('Producto actualizado', 'success');
    cerrarModal('modal-editar-acc');
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al actualizar', 'error'); }
  finally { btn.disabled = false; }
}
...
async function eliminarAccesorio(id, nombre) {
  ...
  try {
    ...
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al eliminar', 'error'); }
}

async function eliminarPadreCompleto(padreId, nombre) {
  if (!confirm('Eliminar el grupo "'+nombre+'" y todas sus variantes?\nEsta accion no se puede deshacer.')) return;
  try {
    ...
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al eliminar el grupo', 'error'); }
}

async function eliminarVariante(padreId, varId) {
  var v = accesoriosCache.find(a => a.id === varId && a._esVariante);
  var nombre = v ? v.nombre_variante : 'esta variante';
  if (!confirm('Eliminar "'+nombre+'"?\nEsta accion no se puede deshacer.')) return;
  try {
    await db.collection('accesorios').doc(padreId).collection('variantes').doc(varId).delete();
    mostrarAlerta('"'+nombre+'" eliminada', 'success');
    cargarAccesorios();
  } catch(err) { console.error(err); mostrarAlerta('Error al eliminar la variante', 'error'); }
}
...
function filtrarAccesorios() {
  ...
  var filtrados = accesoriosCache.filter(a => {
